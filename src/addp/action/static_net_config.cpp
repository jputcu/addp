...
{
    set_request(static_net_config_request(addp::parse_mac_str(mac), _ip, _subnet, _gateway, _password));
}

void static_net_config::set_ip(const std::string& ip)
{
    set_request(static_net_config_request(_mac_address,
										  addp::parse_ip_str(ip), _subnet, _gateway, _password));
}

void static_net_config::set_subnet(const std::string& subnet)
{
    set_request(static_net_config_request(_mac_address, _ip,
										  addp::parse_ip_str(subnet), _gateway, _password));
}

void static_net_config::set_gateway(const std::string& gateway)
{
    set_request(static_net_config_request(_mac_address, _ip, _subnet,
										  addp::parse_ip_str(gateway), _password));
}
