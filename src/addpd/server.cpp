...
server::server(uint16_t port, const std::string &mcast_ip)
    : _io_context(), _listen_address(boost::asio::ip::udp::v4(), port),
      _socket(_io_context, _listen_address), _thread_count(4) {
  _socket.set_option(
      boost::asio::ip::multicast::join_group(boost::asio::ip::make_address(mcast_ip)));
...
void server::stop() { _io_context.stop(); }
